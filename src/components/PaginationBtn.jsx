...
const PaginationBtn = () => {
    ...
    return (
        <>
            <div className="flex bg-buttonSesion rounded-lg font-[Poppins] text-white">
            <button onClick={back} className="h-12 border-2 border-r-0 border-indigo-600
                px-4 rounded-l-lg hover:bg-lightblueColor hover:text-white">
            ...
            pages.map((pg, i) => (
                ...
                w-12 ${cur === pg.page && 'bg-lightblueColor text-black'}`}>{pg.page}</button>
                ))
            }
            <button onClick={Next} className="h-12 border-2  border-indigo-600
                    px-4 rounded-r-lg hover:bg-lightblueColor hover:text-white">
