...
const [showConfirmationEnableEventPopup, setShowConfirmationEnableEventPopup] = useState(false);

const handleConfirmationEnableEventPopup = useCallback((event) => {
  event.preventDefault();
  setShowConfirmationEnableEventPopup(true);
}, []);
...
const handleCancelEnableEvent = useCallback(() => {
  setShowConfirmationEnableEventPopup(false);
}, []);
  ...
  <div key={index}>
    ...
    >
      ...
      <button type="submit" className="bg-green-500 rounded-full py-2 px-8 text-white md:w-40">
         ...
         {/* seccion para el boton de habilitar evento */}
  
   <section className='flex justify-center py-10 mb-8'>
  <div className='bg-hardblue font-bold w-64 rounded-lg py-5 md:w-[75%] md:flex md:justify-center'>
    <button type='button' className='flex items-center justify-center ' onClick={handleConfirmationEnableEventPopup}>
      <img src={PadlockIcon} alt="PadlockIcon" className="w-6 h-6 ml-5 mr-5 "  />
      <span className='text-white'>Habilitar evento</span>
      <img src={PadlockIcon} alt="PadlockIcon" className="w-6 h-6 ml-4" />
    </button>
    {showConfirmationEnableEventPopup && (
      <Popup 
      open={true} 
      position= "center center"
      onClose={handleCancelEnableEvent}>
        <div className="p-4 bg-white flex flex-col items-center justify-center">
          ...
          <p>¿Quiere habilitar el evento?</p>
          <div className="flex justify-end mt-4">
            <button className="bg-white text-blue-500 px-4 py-2 rounded-lg mr-2" onClick={handleCancelEnableEvent}>
              Cancelar
            </button>
...
export default EnableEventForm